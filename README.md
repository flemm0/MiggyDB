...
9 rows 3 columns
┌─────────────┬────────────┬────────────┐
│ employee_id ┆ first_name ┆ is_manager │
│ ---         ┆ ---        ┆ ---        │
│ i64         ┆ str        ┆ bool       │
╞═════════════╪════════════╪════════════╡
│ 100         ┆ Spongebob  ┆ false      │
...
```
🤑> query gimme employee_id:id, first_name:name, is_manager from KrustyKrabEmployees
...
```
🤑> query from KrustyKrabEmployees filter (employee_id gte 102), (last_name in {'Squarepants', 'Krabs', 'Star'})
...
```
🤑> query from KrustyKrabEmployees filter [(employee_id gte 102)], [(is_manager eq True)]
...
```
# (('employee_id' > 102 and 'age' < 30) or (is_manager == True))

🤑> query from KrustyKrabEmployees filter [(employee_id gt 102), (age lt 30)], [(is_manager eq True)]
...
```
🤑 > show tables
Existing tables under demo:

KrustyKrabEmployees
KrustyKrabDepartments
```
```
